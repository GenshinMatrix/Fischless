...
public class LottieAnimationView : SKElement
{
    ...
    public double Seek
    {
        get => double.NaN; // Don't use getter
        set => SetValue(SeekProperty, value);
    }

    public static readonly DependencyProperty SeekProperty =
        DependencyProperty.Register("Seek", typeof(double), typeof(LottieAnimationView), new PropertyMetadata(0d, SeekPropertyChangedCallback));

    private static void SeekPropertyChangedCallback(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        if (d is LottieAnimationView lottieAnimationView)
        {
            lottieAnimationView.animation.Seek((double)e.NewValue);
        }
    }
